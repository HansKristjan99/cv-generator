...
from app.services.quota import ensure_session_available
from app.services.rendering import resolve_template_slug
from app.services.sessions import get_user_session, latest_structured
...
async def generate_cv(
    ...
    if session_id is None:
        ...
        ensure_session_available(db, current_user)
        ...
        cv_session = get_user_session(
            db, session_id, current_user.id, not_found="Unknown or expired conversation."
        )
        ...
        if post_gate:
            ...
            current_doc = latest_structured(db, cv_session.id)
            if current_doc:
                prompt_input = (
                    f"=== CURRENT DOCUMENT ===\n"
                    ...
                    f"{json.dumps(current_doc, indent=2)}\n\n"
                    f"=== USER REQUEST ===\n{user_message}"
                )
            else:
    ...
    template_slug = resolve_template_slug(db, current_user, template_id)
    cv_session.status = "pending"
    cv_session.error = None
    db.add(cv_session)
    db.flush()
