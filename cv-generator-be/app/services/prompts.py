...
CV_SYSTEM_PROMPT = (
    ...
    "All free-text fields (summary, bullets, descriptions, thesis, coursework, skill items, award titles) are "
    "passed directly to the LaTeX renderer without further escaping. You MUST output valid LaTeX in these fields:\n"
    "- Escape special characters yourself: & → \\&, % → \\%, $ → \\$, # → \\#, _ → \\_, { → \\{, } → \\}\n"
    "- Tilde (approximation): use $\\approx$ (e.g. '$\\approx$5ms') or \\textasciitilde{} for a literal tilde\n"
    ...
    "- After each successful call the compiled PDF is attached for your review — check the visual layout.\n"
